...
// Season taglines tuned for the Monon Chalk surface — community/bikes/longevity
// framing, no specialty-bike mentions on the landing. Canonical taglines in
// lib/curriculum/season-colors.ts stay accurate to the actual curriculum and
// surface on the other prototype landings.
...
export default function MononChalkLanding() {
  return (
    <>
        ...
        <section>
          <div className="max-w-5xl mx-auto px-6 py-12 grid sm:grid-cols-2 gap-6">
            {COMMUNITY_EVENTS.map((e) => (
              <Card key={e.kind} color={e.color} eyebrow={e.eyebrow} title={e.title} body={e.body} />
            ))}
          </div>
        </section>
          ...
          <div className="max-w-5xl mx-auto px-6 py-16">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {SEASONS.map((s, i) => (
                <Link
                  key={s.number}
                  href={`${BASE}/episodes#season-${s.number}`}
        ...
        <section className="border-t-4 border-dashed border-[#221E1B]">
          <div className="max-w-5xl mx-auto px-6 py-16">
            <div className="flex items-baseline justify-between gap-4 mb-6">
              ...
              <Link href={`${BASE}/episodes`} className="text-sm font-bold underline underline-offset-4 decoration-[#D33E2D] decoration-2">All 32 →</Link>
            </div>
        ...
        <section className="border-t-4 border-dashed border-[#221E1B]">
          <div className="max-w-5xl mx-auto px-6 py-16">
            ...
            <ul className="flex flex-wrap gap-3">
              {[
                { label: "CentOS Academy · classes", color: "#F4B44A" },
                { label: "Wanderlearn · routes", color: "#5C8AA5" },
                { label: "FlashLearnAI · vocab", color: "#D33E2D" },
                { label: "WitUS Inbox · forms", color: "#3E7C3A" },
                { label: "WitUS Outbox · posts", color: "#F4B44A" },
                { label: "CentOS Travel · rides", color: "#5C8AA5" },
              ].map((b, i) => (
                <span key={i} className="sticker px-4 py-2 text-sm font-bold uppercase tracking-wider" style={{ background: b.color, transform: `rotate(${i % 2 === 0 ? -1 : 1}deg)` }}>
                  {b.label}
                </span>
              ))}
            </ul>
...
function Card({ color, eyebrow, title, body }: { color: string; eyebrow: string; title: string; body: string }) {
  return (
    <div className="border-2 border-[#221E1B] p-6 bg-[#fff8e8]" style={{ boxShadow: `6px 6px 0 ${color}` }}>
      <p className="font-mono text-[11px] uppercase tracking-wider mb-1" style={{ color }}>{eyebrow}</p>
      <p className="font-display text-2xl font-bold text-[#221E1B]">{title}</p>
